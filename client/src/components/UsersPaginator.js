...
const UsersPaginator = () => {
    const {parameters} = useSelector(state => state.listUsers)
    ...
    const listPagesHandler = () => {
        ...
        let link = '/?'
        if(parameters.searchKeyword!=='' || parameters.sortOrder!=='')
            link += `searchKeyword=${parameters.searchKeyword}&sortOrder=${parameters.sortOrder}&`
